...
public class TableUtils
{
	...
	/**
	 * Issue the database statements to create the table associated with a
	 * class.
	 * 
	...
	/**
	 * Create a table if it does not already exist. This is not supported by all
	 * databases.
	 */
	 ...
	 * @param tableConfig
	 *            Hand or spring wired table configuration. If null then the
	 *            class must have {@link DatabaseField} annotations.
	 * @return The number of statements executed to do so.
	 */
	public static <T> int createTable(	ConnectionSource connectionSource,
	...
	/**
	 * Return an ordered collection of SQL statements that need to be run to
	 * create a table. To do the work of creating, you should call
	 * {@link #createTable}.
	 * 
	 * @param connectionSource
	 *            Our connect source which is used to get the database type, not
	 *            to apply the creates.
	 * @param dataClass
	 *            The class for which a table will be created.
																...
																Class<T> dataClass) throws SQLException
	{
		...
		{
			return addCreateTableStatements(connectionSource,
											((BaseDaoImpl<?, ?>) dao).getTableInfo(),
											false);
		} else
		{
																...
																DatabaseTableConfig<T> tableConfig) throws SQLException
	{
		...
		} else
		{
			...
			TableInfo<T, ID> tableInfo = new TableInfo<T, ID>(	connectionSource.getDatabaseType(),
																null,
																tableConfig);
			return addCreateTableStatements(connectionSource, tableInfo, false);
		}
	 ...
	 * 
	 * <p>
	 * <b>WARNING:</b> This is [obviously] very destructive and is
	 * unrecoverable.
	 * </p>
	 * 
	 * @param connectionSource
	 ...
	 * @param ignoreErrors
	 *            If set to true then try each statement regardless of
	 *            {@link SQLException} thrown previously.
	 * @return The number of statements executed to do so.
	 */
										...
										boolean ignoreErrors) throws SQLException
	{
		...
		if (dao instanceof BaseDaoImpl<?, ?>)
		{
			return doDropTable(	databaseType,
								connectionSource,
								((BaseDaoImpl<?, ?>) dao).getTableInfo(),
								ignoreErrors);
		} else
	...
	/**
	 * Issue the database statements to drop the table associated with a table
	 * configuration.
	 * 
	 * <p>
	 * <b>WARNING:</b> This is [obviously] very destructive and is
										...
										boolean ignoreErrors) throws SQLException
	{
		...
		{
			tableConfig.extractFieldTypes(connectionSource);
			TableInfo<T, ID> tableInfo = new TableInfo<T, ID>(	databaseType,
																null,
																tableConfig);
			return doDropTable(	databaseType,
								connectionSource,
	...
	private static <T, ID> int createTable(	ConnectionSource connectionSource,
											Class<T> dataClass,
											boolean ifNotExists) throws SQLException
	{
		...
		if (dao instanceof BaseDaoImpl<?, ?>)
		{
			return doCreateTable(	connectionSource,
									((BaseDaoImpl<?, ?>) dao).getTableInfo(),
									ifNotExists);
		} else
		{
	...
	private static <T> int clearTable(	ConnectionSource connectionSource,
										String tableName) throws SQLException
	{
		...
		try
		{
			compiledStmt = connection.compileStatement(	statement,
	...
	private static <T, ID> int doDropTable(	DatabaseType databaseType,
											...
											boolean ignoreErrors) throws SQLException
	{
		...
		try
		{
			return doStatements(connection,
								"drop",
								statements,
								ignoreErrors,
	...
	private static <T, ID> void addDropIndexStatements(	DatabaseType databaseType,
														TableInfo<T, ID> tableInfo,
														List<String> statements)
	{
		...
		{
			logger.info("dropping index '{}' for table '{}",
						indexName,
	 ...
	 */
	private static <T, ID> void addCreateTableStatements(	DatabaseType databaseType,
															TableInfo<T, ID> tableInfo,
															List<String> statements,
															...
															boolean ifNotExists) throws SQLException
	{
		...
		for (FieldType fieldType : tableInfo.getFieldTypes())
		{
			...
			{
				// we have to call back to the database type for the specific
				// create syntax
				databaseType.appendColumnArg(	tableInfo.getTableName(),
												sb,
												fieldType,
												additionalArgs,
												statementsBefore,
			...
			{
				// hand defined field
				databaseType.appendEscapedEntityName(	sb,
														fieldType.getColumnName());
				sb.append(' ').append(columnDefinition).append(' ');
			}
		}
		// add any sql that sets any primary key fields
		databaseType.addPrimaryKeySql(	tableInfo.getFieldTypes(),
										additionalArgs,
										statementsBefore,
										statementsAfter,
										queriesAfter);
		// add any sql that sets any unique fields
		databaseType.addUniqueComboSql(	tableInfo.getFieldTypes(),
										additionalArgs,
										statementsBefore,
										statementsAfter,
										queriesAfter);
		...
		addCreateIndexStatements(	databaseType,
									tableInfo,
									statements,
									ifNotExists,
									false);
	...
	private static <T, ID> void addCreateIndexStatements(	DatabaseType databaseType,
															TableInfo<T, ID> tableInfo,
															List<String> statements,
															boolean ifNotExists,
															boolean unique)
	{
		...
		{
			logger.info("creating index '{}' for table '{}",
						indexEntry.getKey(),
						tableInfo.getTableName());
			sb.append("CREATE ");
			if (unique)
	 ...
	 */
	private static <T, ID> void addDropTableStatements(	DatabaseType databaseType,
														TableInfo<T, ID> tableInfo,
														List<String> statements)
	{
		List<String> statementsBefore = new ArrayList<String>();
		...
		for (FieldType fieldType : tableInfo.getFieldTypes())
		{
			databaseType.dropColumnArg(	fieldType,
										statementsBefore,
										statementsAfter);
		}
	...
	private static <T, ID> int doCreateTable(	ConnectionSource connectionSource,
												TableInfo<T, ID> tableInfo,
												boolean ifNotExists) throws SQLException
	{
		DatabaseType databaseType = connectionSource.getDatabaseType();
		...
		addCreateTableStatements(	databaseType,
									tableInfo,
									statements,
									queriesAfter,
									ifNotExists);
		...
		try
		{
			int stmtC = doStatements(	connection,
										"create",
										statements,
										false,
										databaseType.isCreateTableReturnsNegative(),
										databaseType.isCreateTableReturnsZero());
			stmtC += doCreateTestQueries(	connection,
											databaseType,
											queriesAfter);
			return stmtC;
	...
	private static int doStatements(DatabaseConnection connection,
									...
									boolean expectingZero) throws SQLException
	{
		int stmtC = 0;
		for (String statement : statements)
		{
			...
			{
				IOUtils.closeThrowSqlException(	compiledStmt,
												"compiled statement");
			}
			// sanity check
			if (rowC < 0)
			{
				if (!returnsNegative)
				{
					throw new SQLException("SQL statement " + statement
							+ " updated " + rowC
	...
	private static int doCreateTestQueries(	DatabaseConnection connection,
											...
											List<String> queriesAfter) throws SQLException
	{
		...
		for (String query : queriesAfter)
		{
			...
			try
			{
				compiledStmt = connection.compileStatement(	query,
				...
				logger.info("executing create table after-query got {} results: {}",
							rowC,
							query);
			} catch (SQLException e)
			{
				// we do this to make sure that the statement is in the
				// exception
				throw SqlExceptionUtil.create("executing create table after-query failed: "
						+ query, e);
			} finally
			{
																	...
																	boolean ifNotExists) throws SQLException
	{
		List<String> statements = new ArrayList<String>();
		List<String> queriesAfter = new ArrayList<String>();
		addCreateTableStatements(	connectionSource.getDatabaseType(),
									tableInfo,
									statements,
									queriesAfter,
									ifNotExists);
